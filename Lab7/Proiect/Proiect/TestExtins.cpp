...
void testColectiaValorilor(Relatie r) {
	MDO d = MDO(r);

	assert(d.vid());
	vector<TValoare> vals = d.colectiaValorilor();
	assert(vals.size() == 0);

	d.adauga(1, 2);
	vals = d.colectiaValorilor();
	assert(vals.size() == 1);
	assert(vals[0] == 2);
...
void testColectiaValorilorFull() {
	testColectiaValorilor(cresc);
	testColectiaValorilor(desc);
}
